...
# print usage at default (no args) start ('cos of cheap way to do
# command-line args)
print '\n***********************************************************************'
print '\n', sys.argv[0],
...
if len(sys.argv) > 1:
    ...
    # why report only Square? (won't often change unless you're in a 'plane!)
    loclen = 6
...
##
            ...
            if ll:
                ...
                sys.stderr.write(
                    sys.argv[0] + ': ERROR :: Cannot determine LAT / LONG. Is ''`gpsd'' running?\n\n')
                sys.exit(44)
                if -180 <= lon < 180:
                    pass
                else:
                    sys.stderr.write(
                        'ERROR :: longitude must be -180<=lon<180\n\n')
                    sys.exit(32)
...
# TO DO
# DONE! Trap no valid GPS Data - if GPS device is unplugged gpsd continues
# but provides `?'
